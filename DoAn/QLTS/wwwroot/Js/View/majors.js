...
$(document).ready(function () {
    ...
    majorJS = new MajorsJS();

})

class MajorsJS extends BaseJS {
    constructor() {
        super();
        this.initEventsPage();
        this.loadOlogy()
    ...
    setDataUrl() {
        this.getDataUrl = "/api/v1/majors/";
        this.getCode = "MajorsCode";
    }
    ...
    filterData() {
        try {
            ...
            listData = cacheData.filter(function (item) {
                return ((item["MajorsName"].toLowerCase()).includes(value.toLowerCase()) || (item["MajorsCode"].toLowerCase()).includes(value.toLowerCase())) && (ologyId ? item["OlogyID"] === ologyId : item["OlogyID"] != ologyId);
            });
            $('.loading').show();
